...
def _load_icons(asset_dir: Path, *, alpha_threshold: int) -> tuple[list[IconBitmap], list[IconBitmap]]:
    ...
    for fp in files:
        ...
        icons_24.append(IconBitmap(name=base_ident, sym_base=sym_base, w=w24, h=h24, stride=stride24, data=data24))
        icons_16.append(IconBitmap(name=base_ident, sym_base=sym_base, w=w16, h=h16, stride=stride16, data=data16))
...
def _write_registry(out_h: Path, out_c: Path, icons_16: list[IconBitmap]) -> None:
    ...
    lines_h.append(" *")
    lines_h.append(' * Accepted forms: "arrow_back", "mi_arrow_back", "mi_arrow_back_16px", "mi_arrow_back_24px".')
    lines_h.append(" * size_px: requested pixel size (e.g. 16 or 24). Nearest available is returned.")
    lines_h.append(" */")
    ...
    for ic in icons_16:
        lines_c.append(
            f'    {{"{ic.name}", &{ic.sym_base}_16px, &{ic.sym_base}_24px}},'
        )
    lines_c.append("};")
    lines_c.append("")
