...
public:
    ...
    /* Buzzer */
    // Buzzer DISABLED (Filip): silence every _tone() caller (view/menu click, encoder test,
    // arkanoid) on the DinMeter in one place. _noTone() still actively silences the pin.
    inline void _tone(unsigned int /*frequency*/, unsigned long /*duration*/ = 0UL) { noTone(BUZZ_PIN); }
