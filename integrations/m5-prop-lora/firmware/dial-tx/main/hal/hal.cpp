...
namespace HAL
{
    ...
    void HAL::_encoder_init()
    {
        ...
        encoder.btn.setDebounce(30);
        encoder.btn.begin();   // FIX self-arm (#8): GPIO42 as a pulled-up INPUT (else it floats,
                               // Button::read() latches a phantom press, and the prop-tx app
                               // spuriously re-runs ACTION_ARM -> the Dial arms itself).
        encoder.btn.setPressedCallback(_encoder_button_pressed_callback, this);
    }
