...
def test_r91_text_int_errors():
    d = _make([{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": 42}])
    es = [e for e in _errors(d) if "text=" in e.message and "must be a string" in e.message]
    assert len(es) == 1


def test_r91_text_list_errors():
    d = _make([{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": ["a", "b"]}])
    es = [e for e in _errors(d) if "text=" in e.message and "must be a string" in e.message]
    assert len(es) == 1


def test_r91_text_string_ok():
    d = _make([{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": "hello"}])
    es = [e for e in _errors(d) if "text=" in e.message and "must be a string" in e.message]
    assert es == []
...
def test_r91_text_none_ok():
    d = _make([{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": None}])
    es = [e for e in _errors(d) if "text=" in e.message and "must be a string" in e.message]
    assert es == []
...
def test_r92_valign_top_on_gauge_warns():
    d = _make(
        [{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 20, "value": 50, "valign": "top"}]
    )
...
def test_r92_valign_bottom_on_box_warns():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 40, "height": 20, "valign": "bottom"}])
    ws = [w for w in _warns(d) if "valign" in w.message and "non-text" in w.message]
    assert len(ws) == 1
...
def test_r93_bar_style_on_label_warns():
    d = _make(
        [{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": "hi", "style": "bar"}]
    )
...
def test_r94_font_size_8_ok():
    d = _make(
        [{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": "hi", "font_size": 8}]
    )
    es = [e for e in _errors(d) if "font_size" in e.message and "uint8" in e.message]
    assert es == []
...
def test_r95_runtime_int_errors():
    d = _make(
        [{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": "hi", "runtime": 123}]
    )
...
def test_r98_corner_radius_exceeds_warns():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 20, "height": 20, "corner_radius": 15}])
    ws = [w for w in _warns(d) if "corner_radius" in w.message and "exceeds" in w.message]
    assert len(ws) == 1


def test_r98_corner_radius_ok():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 20, "height": 20, "corner_radius": 10}])
    ws = [w for w in _warns(d) if "corner_radius" in w.message and "exceeds" in w.message]
    assert ws == []
