...
def test_r75_chart_too_short():
    d = _make([{"type": "chart", "x": 0, "y": 0, "width": 40, "height": 10,
                "data_points": [1, 2, 3], "style": "bar"}])
    ws = [w for w in _warns(d) if "chart" in w.message and "too small" in w.message]
    assert len(ws) == 1
...
def test_r76_border_width_with_border_true():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 40, "height": 20,
                "border": True, "border_style": "single", "border_width": 2}])
...
def test_r77_empty_overflow_on_non_text_ok():
    d = _make([{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 40,
                "text_overflow": "", "value": 0, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "text_overflow" in w.message and "non-text" in w.message]
    assert ws == []
...
def test_r78_align_on_gauge():
    d = _make([{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 40,
                "align": "center", "value": 0, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "align=" in w.message and "non-text" in w.message]
    assert len(ws) == 1
...
def test_r78_left_align_on_gauge_ok():
    d = _make([{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 40,
                "align": "left", "value": 0, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "align=" in w.message and "non-text" in w.message]
    assert ws == []
...
def test_r78_empty_align_on_gauge_ok():
    d = _make([{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 40,
                "align": "", "value": 0, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "align=" in w.message and "non-text" in w.message]
    assert ws == []
...
def test_r79_widget_taller_than_scene():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 40, "height": 100}],
              scene_w=128, scene_h=64)
...
def test_r80_margin_x_pushes_offscreen():
    d = _make([{"type": "box", "x": 100, "y": 10, "width": 20, "height": 20,
                "margin_x": 30}], scene_w=128, scene_h=64)
    ws = [w for w in _warns(d) if "margin_x" in w.message and "past scene" in w.message]
    assert len(ws) == 1


def test_r80_margin_y_pushes_offscreen():
    d = _make([{"type": "box", "x": 10, "y": 50, "width": 20, "height": 20,
                "margin_y": 20}], scene_w=128, scene_h=64)
...
def test_r81_progressbar_no_text_ok():
    d = _make([{"type": "progressbar", "x": 0, "y": 0, "width": 60, "height": 14,
...
def test_r81_progressbar_with_text():
    d = _make([{"type": "progressbar", "x": 0, "y": 0, "width": 60, "height": 14,
                "text": "LOADING", "value": 50, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "progressbar" in w.message and "not rendered" in w.message]
    assert len(ws) == 1
...
def test_r81_progressbar_whitespace_ok():
    d = _make([{"type": "progressbar", "x": 0, "y": 0, "width": 60, "height": 14,
                "text": "  ", "value": 50, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "progressbar" in w.message and "not rendered" in w.message]
    assert ws == []
...
def test_r82_slider_value_ok():
    d = _make([{"type": "slider", "x": 0, "y": 0, "width": 60, "height": 16,
                "value": 50, "min_value": 0, "max_value": 100}])
    ws = [w for w in _warns(d) if "not a value widget" in w.message]
    assert ws == []
