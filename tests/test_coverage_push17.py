...
class TestConstructorEnvBranches:
    ...
    def test_bad_live_baud(self, tmp_path, monkeypatch, make_app):
    ...
    def test_bad_autosave_secs(self, tmp_path, monkeypatch, make_app):
    ...
    def test_bad_fps_env(self, tmp_path, monkeypatch, make_app):
    ...
    def test_bad_max_scale_env(self, tmp_path, monkeypatch, make_app):
    ...
    def test_prefs_profile_loaded(self, tmp_path, monkeypatch, make_app):
    ...
    def test_prefs_live_port_loaded(self, tmp_path, monkeypatch, make_app):
    ...
    def test_prefs_bad_live_baud(self, tmp_path, monkeypatch, make_app):
    ...
    def test_overflow_warn_on(self, tmp_path, monkeypatch, make_app):
    ...
    def test_overflow_warn_off(self, tmp_path, monkeypatch, make_app):
    ...
    def test_autosave_enabled(self, tmp_path, monkeypatch, make_app):
...
class TestZOrderMethods:
    def test_z_order_step_forward(self, make_app):
    ...
    def test_z_order_step_backward(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app._z_order_step(-1)
        w = app.state.current_scene().widgets[0]
    ...
    def test_z_order_bring_to_front(self, make_app):
    ...
    def test_z_order_send_to_back(self, make_app):
        widgets = [_w(x=0, y=0), _w(x=8, y=8)]
        app = make_app(widgets=widgets)
    ...
    def test_z_order_step_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._z_order_step(1)  # early return, no crash

    def test_z_order_front_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._z_order_bring_to_front()

    def test_z_order_back_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._z_order_send_to_back()
...
class TestToggleLock:
    def test_toggle_lock_on(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_toggle_lock_off(self, make_app):
        w = _w()
        w.locked = True
        app = make_app(widgets=[w])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app._toggle_lock_selection()
        assert app.state.current_scene().widgets[0].locked is False

    def test_toggle_lock_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._toggle_lock_selection()
...
class TestSwitchScene:
    def test_switch_next(self, make_app):
    ...
    def test_switch_prev_wraps(self, make_app):
        app = make_app(extra_scenes=True)
        names = list(app.designer.scenes.keys())
        app.designer.current_scene = names[0]
        app._switch_scene(-1)
    ...
    def test_switch_single_scene(self, make_app):
        app = make_app()
...
class TestOptimizedDraw:
    def test_canvas_only_dirty(self, make_app):
    ...
    def test_clean_preview_draw(self, make_app):
    ...
    def test_full_redraw(self, make_app):
    ...
    def test_help_overlay_during_draw(self, make_app):
    ...
    def test_vsync_disabled(self, make_app):
...
class TestHelpOverlay:
    def test_maybe_hide_help_not_shown(self, make_app):
        app = make_app()
        app._maybe_hide_help_overlay()

    def test_maybe_hide_help_pinned(self, make_app):
        app = make_app()
        app.show_help_overlay = True
    ...
    def test_maybe_hide_help_timeout(self, make_app):
        app = make_app()
        app.show_help_overlay = True
        app._help_shown_once = False
    ...
    def test_set_help_overlay_on_pinned(self, make_app):
        app = make_app()
    ...
    def test_set_help_overlay_off(self, make_app):
        app = make_app()
        app.show_help_overlay = True
        app._help_pinned = True
    ...
    def test_set_help_overlay_no_change(self, make_app):
        app = make_app()
        app._help_pinned = False
        app._set_help_overlay(False)  # no-op

    def test_toggle_help_overlay_from_off(self, make_app):
        app = make_app()
        app._toggle_help_overlay()
        assert app.show_help_overlay
        assert app._help_pinned

    def test_toggle_help_overlay_pinned_off(self, make_app):
        app = make_app()
        app.show_help_overlay = True
        app._help_pinned = True
        app._toggle_help_overlay()
    ...
    def test_toggle_help_overlay_unpinned_pin(self, make_app):
        app = make_app()
        app.show_help_overlay = True
        app._help_pinned = False
        app._toggle_help_overlay()
...
class TestInspectorEdit:
    def test_start_edit_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app.state.selected_idx = None
        app._inspector_start_edit("text")

    def test_start_edit_with_widget(self, make_app):
    ...
    def test_cancel_edit(self, make_app):
        app = make_app()
    ...
    def test_on_text_input_with_field(self, make_app):
        app = make_app()
        app.state.inspector_selected_field = "text"
...
class TestColorPresets:
    def test_apply_color_preset_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._apply_color_preset("#fff", "#000")

    def test_apply_color_preset_with_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
    ...
    def test_apply_preset_index_oob(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
...
class TestSceneManagementDeep:
    ...
    def test_delete_switches_to_adjacent(self, make_app):
        app = make_app(extra_scenes=True)
        names = list(app.designer.scenes.keys())
        app.designer.current_scene = names[0]
        app._delete_current_scene()
    ...
    def test_close_other_scenes_multiple(self, make_app):
        app = make_app(extra_scenes=True)
    ...
    def test_close_scenes_to_right_from_first(self, make_app):
        app = make_app(extra_scenes=True)
        names = list(app.designer.scenes.keys())
        app.designer.current_scene = names[0]
    ...
    def test_duplicate_scene_naming(self, make_app):
        app = make_app()
    ...
    def test_add_new_scene_naming(self, make_app):
        app = make_app()
        app._add_new_scene()
        app._add_new_scene()
        names = list(app.designer.scenes.keys())
...
class TestAddWidget:
    ...
    def test_add_widget_type(self, kind, make_app):
        app = make_app()
...
class TestExportCHeader:
    def test_export_success(self, tmp_path, make_app):
        app = make_app(widgets=[_w()])
    ...
    def test_export_codegen_import_error(self, tmp_path, make_app):
        app = make_app()
        app.json_path = tmp_path / "design.json"
        app.save_json()
...
class TestOnKeyDown:
    def test_escape_dismisses_context_menu(self, make_app):
        app = make_app()
    ...
    def test_other_key_with_menu(self, make_app):
        app = make_app()
        app._context_menu = {"visible": True, "pos": (0, 0), "items": [], "hitboxes": []}
...
class TestAutoCompleteWidget:
    def test_label_with_device_profile(self, make_app):
        app = make_app()
    ...
    def test_label_without_device_profile(self, make_app):
        app = make_app()
...
class TestIntelligentAutoArrange:
    def test_with_multiple_types(self, make_app):
        widgets = [
...
class TestDrawingDelegates:
    def test_draw_canvas(self, make_app):
        app = make_app(widgets=[_w()])
        app._draw_canvas()

    def test_draw_toolbar(self, make_app):
        app = make_app()
        app._draw_toolbar()

    def test_draw_scene_tabs(self, make_app):
        app = make_app()
        app._draw_scene_tabs()

    def test_draw_palette(self, make_app):
        app = make_app()
        app._draw_palette()

    def test_draw_inspector(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app._draw_inspector()

    def test_draw_status(self, make_app):
        app = make_app()
        app._draw_status()

    def test_text_width_px(self, make_app):
        app = make_app()
        w = app._text_width_px("Hello")
        assert w > 0

    def test_ellipsize_text(self, make_app):
        app = make_app()
        r = app._ellipsize_text_px("Hello World", 30)
        assert isinstance(r, str)

    def test_wrap_text(self, make_app):
        app = make_app()
        lines = app._wrap_text_px("Hello World Test", 40, 2)
        assert isinstance(lines, list)

    def test_draw_border_style(self, make_app):
        app = make_app()
        surf = pygame.Surface((100, 50))
        app._draw_border_style(surf, pygame.Rect(0, 0, 100, 50), "solid", (255, 255, 255))

    def test_draw_bevel_frame(self, make_app):
        app = make_app()
        surf = pygame.Surface((100, 50))
...
class TestComponentGroupHelpers:
    def test_component_info_from_group_not_comp(self, make_app):
        app = make_app()
        result = app._component_info_from_group("regular_group")
        assert result is None

    def test_component_info_from_group_comp(self, make_app):
        app = make_app()
        result = app._component_info_from_group("comp:header_bar:root")
        assert result is not None

    def test_component_role_index(self, make_app):
        w1 = _w()
    ...
    def test_component_role_index_empty_prefix(self, make_app):
        app = make_app(widgets=[_w()])
        roles = app._component_role_index([0], "")
        assert roles == {}

    def test_selected_component_group_none(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_selected_group_exact_no_match(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_format_group_label_regular(self, make_app):
        app = make_app()
        lbl = app._format_group_label("mygroup", [0, 1])
        assert "mygroup" in lbl

    def test_format_group_label_component(self, make_app):
        app = make_app()
        lbl = app._format_group_label("comp:header_bar:root", [0, 1])
        assert "header_bar" in lbl

    def test_next_group_name(self, make_app):
        app = make_app()
        name = app._next_group_name("group")
        assert name == "group1"

    def test_group_selection_too_few(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app._group_selection()  # needs 2+

    def test_group_and_ungroup(self, make_app):
    ...
    def test_ungroup_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._ungroup_selection()

    def test_tri_state(self, make_app):
        app = make_app()
...
class TestContentHeights:
    def test_palette_content_height(self, make_app):
        app = make_app(widgets=[_w()])
        h = app._palette_content_height()
        assert h > 0

    def test_inspector_content_height(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_inspector_content_height_collapsed(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
...
class TestScaleOps:
    def test_set_scale(self, make_app):
        app = make_app()
        app._set_scale(2)
        assert app._scale_locked

    def test_compute_scale(self, make_app):
        app = make_app()
        s = app._compute_scale()
        assert s >= 1

    def test_recompute_scale(self, make_app):
        app = make_app()
        app._recompute_scale_for_window(800, 600)

    def test_rebuild_layout(self, make_app):
        app = make_app()
...
class TestPrompts:
    ...
    def test_array_dup_prompt_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._array_duplicate_prompt()

    def test_array_dup_prompt_with_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app._array_duplicate_prompt()
...
class TestIsValidColor:
    ...
    def test_hex_valid(self, make_app):
        app = make_app()
        assert app._is_valid_color_str("#aabbcc") is True

    def test_hex_0x_valid(self, make_app):
        app = make_app()
        assert app._is_valid_color_str("0xaabbcc") is True

    def test_hex_invalid(self, make_app):
        app = make_app()
...
class TestAutosavePrefs:
    def test_maybe_autosave_not_dirty(self, make_app):
        app = make_app()
        app._dirty_scenes.clear()
        app._maybe_autosave()

    def test_save_prefs(self, make_app):
        app = make_app()
        app._save_prefs()

    def test_load_prefs(self, make_app):
        app = make_app()
        app._load_prefs()
...
class TestSnapHelpers:
    def test_apply_snap_enabled(self, make_app):
        app = make_app()
        app.snap_enabled = True
        v = app._apply_snap(13)
        assert v % GRID == 0

    def test_apply_snap_disabled(self, make_app):
        app = make_app()
...
class TestLoadSave:
    def test_save_and_load(self, tmp_path, make_app):
        app = make_app(widgets=[_w(text="saved")])
    ...
    def test_widget_presets(self, make_app):
        app = make_app()
        presets = app._load_widget_presets()
        assert isinstance(presets, list)
        app._save_widget_presets()

    def test_build_widget_presets_actions(self, make_app):
        app = make_app()
        actions = app._build_widget_presets_actions()
        assert len(actions) > 0

    def test_build_template_actions(self, make_app):
        app = make_app()
...
class TestDispatchQuit:
    def test_quit_first_press(self, make_app):
        app = make_app()
    ...
    def test_quit_double_press(self, make_app):
        app = make_app()
        app._dirty_scenes = {"main"}
    ...
    def test_quit_no_changes(self, make_app):
        app = make_app()
        app._dirty_scenes.clear()
        ev = SimpleNamespace(type=pygame.QUIT)
        app._dispatch_event(ev)
...
class TestFocusNav:
    def test_is_widget_focusable(self, make_app):
        app = make_app()
        assert app._is_widget_focusable(_w(type="button"))

    def test_focusable_indices(self, make_app):
        widgets = [_w(type="button"), _w(type="label")]
        app = make_app(widgets=widgets)
        indices = app._focusable_indices()
        assert isinstance(indices, list)

    def test_set_focus(self, make_app):
        app = make_app(widgets=[_w(type="button")])
        app._set_focus(0)

    def test_ensure_focus(self, make_app):
        app = make_app(widgets=[_w(type="button")])
        app._ensure_focus()

    def test_focus_cycle(self, make_app):
        widgets = [_w(type="button"), _w(type="slider")]
        app = make_app(widgets=widgets)
        app._focus_cycle(1)

    def test_focus_move_direction(self, make_app):
        widgets = [_w(type="button", x=0, y=0), _w(type="button", x=80, y=0)]
        app = make_app(widgets=widgets)
        app._set_focus(0)
        app._focus_move_direction("right")

    def test_adjust_focused_value(self, make_app):
        app = make_app(widgets=[_w(type="slider", value=50)])
        app._set_focus(0)
        app._adjust_focused_value(5)

    def test_activate_focused(self, make_app):
