...
class TestToggleCleanPreview:
    def test_enter_preview(self, make_app):
        app = make_app()
    ...
    def test_exit_preview_restores_grid(self, make_app):
        app = make_app()
        app.show_grid = True
        app.clean_preview = False
    ...
    def test_toggle_twice_returns_to_original(self, make_app):
        app = make_app()
    ...
    def test_clears_selection_on_enter(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app.clean_preview = False
...
class TestTogglePanels:
    ...
    def test_double_toggle(self, make_app):
        app = make_app()
...
class TestResetZoom:
    def test_resets_to_one(self, make_app):
        app = make_app()
        app._reset_zoom()
        assert app.scale == 1
...
class TestApplySnap:
    def test_snap_enabled(self, make_app):
        app = make_app()
        app.snap_enabled = True
        result = app._apply_snap(13)
        assert result % 8 == 0

    def test_snap_disabled(self, make_app):
        app = make_app()
        app.snap_enabled = False
        assert app._apply_snap(13) == 13

    def test_already_snapped(self, make_app):
        app = make_app()
        app.snap_enabled = True
...
class TestZOrderStep:
    def test_step_forward(self, make_app):
        app = make_app()
        w = _add(app)
        w.z_index = 0
        _sel(app, 0)
    ...
    def test_step_backward(self, make_app):
        app = make_app()
        w = _add(app)
        w.z_index = 5
        _sel(app, 0)
    ...
    def test_no_selection_no_change(self, make_app):
        app = make_app()
        w = _add(app)
        w.z_index = 3
        # No selection
    ...
    def test_multiple_selected(self, make_app):
        app = make_app()
        w0 = _add(app)
        w1 = _add(app)
        w0.z_index = 0
...
class TestCycleStyle:
    def test_cycle_with_selection(self, make_app):
        app = make_app()
...
class TestCycleWidgetType:
    def test_cycle_type(self, make_app):
        app = make_app()
        w = _add(app, type="label")
        _sel(app, 0)
...
class TestCycleBorderStyle:
    def test_cycle_border(self, make_app):
        app = make_app()
...
class TestToggleVisibility:
    def test_toggle_hidden(self, make_app):
        app = make_app()
        w = _add(app)
        w.visible = True
        _sel(app, 0)
    ...
    def test_toggle_visible(self, make_app):
        app = make_app()
        w = _add(app)
        w.visible = False
        _sel(app, 0)
...
class TestSnapSelectionToGrid:
    def test_snaps_coords(self, make_app):
        app = make_app()
...
class TestCenterInScene:
    def test_centers_single(self, make_app):
        app = make_app()
...
class TestSetSelection:
    def test_set_single(self, make_app):
        app = make_app()
        _add(app)
        app._set_selection([0], anchor_idx=0)
        assert 0 in app.state.selected

    def test_set_multiple(self, make_app):
        app = make_app()
        _add(app)
        _add(app)
    ...
    def test_clear_selection(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._set_selection([])
    ...
    def test_anchor_idx(self, make_app):
        app = make_app()
        _add(app)
        _add(app)
...
class TestScreenToLogical:
    def test_returns_tuple(self, make_app):
        app = make_app()
        result = app._screen_to_logical((100, 50))
        assert isinstance(result, tuple)
        assert len(result) == 2

    def test_origin(self, make_app):
        app = make_app()
...
class TestContextActionsExtended:
    def test_toggle_visibility_action(self, make_app):
        app = make_app()
        w = _add(app)
        w.visible = True
        _sel(app, 0)
    ...
    def test_cycle_style_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._execute_context_action("cycle_style")

    def test_cycle_type_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._execute_context_action("cycle_type")

    def test_cycle_border_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
    ...
    def test_reorder_down_action(self, make_app):
        app = make_app()
        _add(app)
        _add(app)
        _sel(app, 0)
    ...
    def test_center_in_scene_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._execute_context_action("center_in_scene")

    def test_snap_to_grid_action(self, make_app):
        app = make_app()
        _add(app, x=13, y=17)
        _sel(app, 0)
        app._execute_context_action("snap_to_grid")

    def test_smart_edit_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._execute_context_action("smart_edit")

    def test_toggle_enabled_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
        app._execute_context_action("toggle_enabled")

    def test_mirror_action(self, make_app):
        app = make_app()
        _add(app)
        _sel(app, 0)
    ...
    def test_stack_vertical_action(self, make_app):
        app = make_app()
    ...
    def test_stack_horizontal_action(self, make_app):
        app = make_app()
        _add(app, x=0, y=0, width=40, height=16)
        _add(app, x=0, y=0, width=40, height=16)
        _sel(app, 0, 1)
