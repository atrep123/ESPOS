...
def test_r107_wrap_maxlines2_ok():
    ...
    ws = [
        w
        for w in _warns(d)
        if "wrap" in w.message and "max_lines=" in w.message and "second line" in w.message
    ]
    assert len(ws) == 0
...
def test_r108_slider_tall_warns():
    ...
    ws = [
        w
        for w in _warns(d)
        if "slider" in w.message and "height" in w.message and "width" in w.message
    ]
    assert len(ws) == 1
...
def test_r111_border_false_no_style_ok():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 30, "height": 20, "border": False}])
    ws = [w for w in _warns(d) if "style is ignored" in w.message]
    assert len(ws) == 0
