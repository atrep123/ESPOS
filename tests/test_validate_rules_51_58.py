...
def test_rule51_constraints_dict_ok():
    w = [{"type": "label", "x": 0, "y": 0, "width": 40, "height": 14, "constraints": {"ax": "left"}}]
    errs = _errors(_make(w))
...
def test_rule53_parent_id_missing_ref():
    w = [
        {"type": "label", "x": 0, "y": 0, "width": 40, "height": 14, "_widget_id": "lbl1", "parent_id": "panel99"},
    ]
...
def test_rule53_parent_id_valid_ref():
    w = [
        ...
        {"type": "label", "x": 2, "y": 2, "width": 40, "height": 14, "_widget_id": "lbl1", "parent_id": "pnl1"},
    ]
    warns = _warns(_make(w))
...
def test_rule57_value_fields_on_gauge_ok():
    w = [{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 30, "min_value": 0, "max_value": 100}]
    warns = _warns(_make(w))
...
def test_rule57_zero_values_ok():
    ...
    w = [{"type": "label", "x": 0, "y": 0, "width": 40, "height": 14, "min_value": 0, "max_value": 0}]
    warns = _warns(_make(w))
    assert not any("on non-value" in i.message for i in warns)
