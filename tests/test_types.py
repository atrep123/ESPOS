...
from ui_models import WidgetConfig, _coerce_int, make_baseline
...
def testmake_baseline_accepts_optional(width, height):
    b = make_baseline(1, 2, width, height, None, None)
    assert b["x"] == 1
    assert b["y"] == 2
    assert b["width"] >= 0
    assert b["height"] >= 0
