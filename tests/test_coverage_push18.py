...
class TestOnTextInputEmpty:
    def test_empty_text_returns_early(self, make_app):
        app = make_app()
...
class TestExceptBranches:
    def test_inspector_start_edit_start_text_input_throws(self, make_app):
    ...
    def test_z_order_step_save_state_throws(self, make_app):
    ...
    def test_z_order_front_save_state_throws(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app.designer._save_state = MagicMock(side_effect=TypeError("boom"))
    ...
    def test_z_order_back_save_state_throws(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
    ...
    def test_toggle_lock_save_state_throws(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app.designer._save_state = MagicMock(side_effect=TypeError("boom"))
    ...
    def test_zoom_to_fit_exception(self, make_app):
    ...
    def test_apply_color_preset_save_state_throws(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app.designer._save_state = MagicMock(side_effect=TypeError("boom"))
    ...
    def test_group_selection_save_state_throws(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
...
class TestUncalledDelegates:
    ...
    def test_draw_text_in_rect(self, make_app):
        app = make_app(widgets=[_w()])
        surf = pygame.Surface((200, 40))
        rect = pygame.Rect(0, 0, 200, 40)
        w = app.state.current_scene().widgets[0]
    ...
    def test_draw_widget_preview(self, make_app):
        app = make_app(widgets=[_w()])
        surf = pygame.Surface((200, 40))
        rect = pygame.Rect(0, 0, 200, 40)
        w = app.state.current_scene().widgets[0]
...
class TestApplyColorPresetBody:
    def test_apply_preset_sets_colors(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
    ...
    def test_apply_preset_index_valid(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
...
class TestSetProfileBody:
    def test_set_profile_valid(self, make_app):
...
class TestAutoAdjustQuality:
    def test_reduce_quality_grid(self, make_app):
        app = make_app()
    ...
    def test_reduce_quality_scale(self, make_app):
        app = make_app()
        app.auto_optimize = True
        app.min_acceptable_fps = 30
        app.show_grid = False
    ...
    def test_reduce_quality_panels(self, make_app):
        app = make_app()
        app.auto_optimize = True
        app.min_acceptable_fps = 30
        app.show_grid = False
    ...
    def test_increase_quality(self, make_app):
        app = make_app()
        app.auto_optimize = True
        app.min_acceptable_fps = 30
        app.show_grid = False
    ...
    def test_no_adjust_short_history(self, make_app):
        app = make_app()
        app.auto_optimize = True
...
class TestOptimizedDrawCanvas:
    def test_canvas_only_dirty_rect(self, make_app):
        app = make_app(widgets=[_w()])
        app._dirty = True
...
class TestGroupHelperExceptions:
    def test_groups_for_index_member_check_raises(self, make_app):
        app = make_app(widgets=[_w()])
    ...
    def test_groups_for_index_with_match(self, make_app):
        app = make_app(widgets=[_w()])
    ...
    def test_group_members_valid(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
    ...
    def test_next_group_name_existing(self, make_app):
        app = make_app()
...
class TestSelectedGroupExact:
    ...
    def test_no_match(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.designer.groups = {"grp1": [0, 1]}
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_selected_component_group_match(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
    ...
    def test_selected_component_group_no_match(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        result = app._selected_component_group()
    ...
    def test_component_info_non_comp(self, make_app):
        app = make_app()
        result = app._component_info_from_group("regular_group")
        assert result is None

    def test_component_info_valid_comp(self, make_app):
        app = make_app()
...
class TestGroupUngroup:
    def test_group_selection_succeeds(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
        app._group_selection()
    ...
    def test_group_selection_fails(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
    ...
    def test_group_selection_create_raises(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
    ...
    def test_group_single_widget(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app._group_selection()  # too few → status msg

    def test_ungroup_with_group(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
        app._group_selection()
    ...
    def test_ungroup_no_group(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app._ungroup_selection()

    def test_ungroup_empty_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._ungroup_selection()

    def test_ungroup_intersection_fallback(self, make_app):
        """Ungroup when primary_group is None but intersection finds groups."""
        app = make_app(widgets=[_w(), _w(x=8), _w(x=16)])
        app.designer.groups = {"grp1": [0, 1]}
        app.state.selected = [0, 2]
        app.state.selected_idx = 2  # index 2 is NOT in grp1
        app._ungroup_selection()

    def test_ungroup_delete_group_raises(self, make_app):
        app = make_app(widgets=[_w(), _w(x=8)])
        app.designer.groups = {"grp1": [0, 1]}
        app.state.selected = [0, 1]
        app.state.selected_idx = 0
...
class TestHandleEvents:
    def test_handle_events_basic(self, make_app):
        app = make_app()
        # Pump some events then process them
        pygame.event.clear()
        app._handle_events()

    def test_handle_events_with_quit(self, make_app):
        app = make_app()
        pygame.event.clear()
...
class TestOpenContextMenu:
    def test_context_menu_non_rect_scene_rect(self, make_app):
        app = make_app(widgets=[_w()])
    ...
    def test_context_menu_hit_widget(self, make_app):
        app = make_app(widgets=[_w(x=0, y=0, width=60, height=20)])
    ...
    def test_context_menu_with_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        sr = app.layout.canvas_rect
    ...
    def test_context_menu_with_clipboard(self, make_app):
        app = make_app(widgets=[_w()])
    ...
    def test_open_tab_context_menu_with_hitboxes(self, make_app):
    ...
    def test_open_tab_context_menu_close_right(self, make_app):
...
class TestClickContextMenu:
    ...
    def test_click_misses_hitbox(self, make_app):
        app = make_app()
        hitbox = pygame.Rect(10, 10, 100, 20)
        app._context_menu = {
            "visible": True,
    ...
    def test_click_no_menu(self, make_app):
        app = make_app()
...
class TestExecuteContextActionDeep:
    ...
    def test_smart_edit(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        app._execute_context_action("smart_edit")

    def test_edit_text(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_view_toggles(self, make_app, action):
        app = make_app()
    ...
    def test_add_widget_actions(self, make_app, action):
        app = make_app()
...
# ===========================================================================
class TestZoomToFit:
    def test_zoom_to_fit_normal(self, make_app):
        app = make_app(widgets=[_w()])
        app._zoom_to_fit()

    def test_zoom_to_fit_zero_canvas(self, monkeypatch, make_app):
        app = make_app()
...
class TestSwitchSceneError:
    def test_switch_with_invalid_current(self, make_app):
        app = make_app(extra_scenes=True)
...
class TestHandleDoubleClickDeep:
    def test_double_click_on_tab(self, make_app):
        app = make_app()
    ...
    def test_double_click_on_tab_no_hit(self, make_app):
        app = make_app()
        tab_rect = app.layout.scene_tabs_rect
    ...
    def test_double_click_non_rect_scene(self, make_app):
    ...
    def test_double_click_on_widget(self, make_app):
        app = make_app(widgets=[_w(x=0, y=0, width=60, height=20)])
        sr = app.layout.canvas_rect
        pos = (sr.x + 5, sr.y + 5)
        app._handle_double_click(pos)
    ...
    def test_double_click_outside_scene(self, make_app):
        app = make_app()
        # Click far outside
        app._handle_double_click((9999, 9999))

    def test_double_click_no_hit(self, make_app):
        app = make_app()
        sr = app.layout.canvas_rect
...
class TestPromptMethods:
    ...
    def test_array_duplicate_prompt_with_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_array_duplicate_prompt_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._array_duplicate_prompt()

    def test_set_all_spacing_prompt_with_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_set_all_spacing_prompt_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._set_all_spacing_prompt()
...
class TestSaveTemplate:
    def test_save_template_with_valid_selection(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
    ...
    def test_save_template_out_of_range_indices(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [99]  # out of range
        app._save_selection_as_template()
        # widgets list is empty → early return

    def test_save_template_no_selection(self, make_app):
        app = make_app()
        app.state.selected = []
        app._save_selection_as_template()

    def test_save_template_start_text_input_throws(self, make_app):
        app = make_app(widgets=[_w()])
        app.state.selected = [0]
        app.state.selected_idx = 0
        with patch.object(pygame.key, "start_text_input", side_effect=AttributeError("mock")):
...
class TestSceneManagementBodies:
    def test_delete_scene_body(self, make_app):
        app = make_app(extra_scenes=True)
        names = list(app.designer.scenes.keys())
        app.designer.current_scene = names[0]
    ...
    def test_close_other_single_scene(self, make_app):
        """Close others when only one scene exists → early return (L2868)."""
        app = make_app()
        app._close_other_scenes()
        assert len(app.designer.scenes) == 1

    def test_close_other_multiple(self, make_app):
        app = make_app(extra_scenes=True)
    ...
    def test_close_to_right_at_end(self, make_app):
    ...
    def test_close_to_right_body(self, make_app):
        app = make_app(extra_scenes=True)
        app.designer.create_scene("third")
        sc3 = app.designer.scenes["third"]
        sc3.width, sc3.height = 256, 128
    ...
    def test_add_new_scene_while_loop(self, make_app):
    ...
    def test_add_new_scene_simple(self, make_app):
        app = make_app()
        app._add_new_scene()
        assert len(app.designer.scenes) > 1

    def test_duplicate_scene_body(self, make_app):
        app = make_app(widgets=[_w()])
        before = len(app.designer.scenes)
    ...
    def test_duplicate_scene_copy_naming(self, make_app):
        app = make_app()
        app._duplicate_current_scene()
        app._duplicate_current_scene()
        names = list(app.designer.scenes.keys())
    ...
    def test_rename_scene_body(self, make_app):
        app = make_app()
        app._rename_current_scene()
        assert app.state.inspector_selected_field == "_scene_name"

    def test_export_c_header_no_json(self, make_app):
        app = make_app()
        app.json_path = None
        app._export_c_header()

    def test_export_c_header_json_not_exists(self, tmp_path, make_app):
        app = make_app()
        app.json_path = tmp_path / "nonexistent.json"
        app._export_c_header()

    def test_export_c_header_import_error(self, tmp_path, make_app):
        app = make_app()
    ...
    def test_export_c_header_success(self, tmp_path, make_app):
        app = make_app()
        json_file = tmp_path / "test.json"
...
class TestToggleCleanPreview:
    def test_toggle_on(self, make_app):
        app = make_app()
    ...
    def test_toggle_off(self, make_app):
        app = make_app()
        # First turn on
        app.clean_preview = False
        app.show_grid = True
    ...
    def test_toggle_off_restores_state(self, make_app):
        app = make_app()
        app.clean_preview = False
        app.show_grid = True
...
class TestZOrderBackNegativeZ:
    def test_send_to_back_with_existing_negative(self, make_app):
        w1 = _w(x=0, y=0)
        w2 = _w(x=8, y=0)
...
class TestRestoredFromAutosave:
    def test_autosave_flag_print(self, tmp_path, monkeypatch, make_app, capsys):
...
class TestAddWidgetAutoComplete:
    def test_add_widget_auto_complete_exception(self, make_app):
        app = make_app()
    ...
    def test_add_widget_find_position_exception(self, make_app):
        app = make_app()
...
class TestIsValidColorDeep:
    def test_0x_prefix_valid(self, make_app):
        app = make_app()
        assert app._is_valid_color_str("0xAABBCC") is True

    def test_0x_prefix_invalid(self, make_app):
        app = make_app()
        assert app._is_valid_color_str("0xGGHHII") is False

    def test_return_false_garbage(self, make_app):
        app = make_app()
...
class TestDispatchEventPaths:
    def test_dispatch_textinput(self, make_app):
        app = make_app()
        app.state.inspector_selected_field = "text"
    ...
    def test_dispatch_mousewheel(self, make_app):
        app = make_app()
        evt = pygame.event.Event(pygame.MOUSEWHEEL, x=0, y=1)
        app._dispatch_event(evt)

    def test_dispatch_mousemotion(self, make_app):
        app = make_app()
        evt = pygame.event.Event(pygame.MOUSEMOTION, pos=(50, 50), buttons=(0, 0, 0))
        app._dispatch_event(evt)

    def test_dispatch_mousebutton_up(self, make_app):
        app = make_app()
        evt = pygame.event.Event(pygame.MOUSEBUTTONUP, button=1, pos=(50, 50))
        app._dispatch_event(evt)

    def test_dispatch_left_click_with_context_menu(self, make_app):
        app = make_app()
        hitbox = pygame.Rect(10, 10, 100, 20)
        app._context_menu = {
            "visible": True,
    ...
    def test_dispatch_double_click(self, make_app):
        app = make_app(widgets=[_w(x=0, y=0, width=60, height=20)])
    ...
    def test_dispatch_right_click_tab(self, make_app):
        app = make_app(extra_scenes=True)
        tab_rect = app.layout.scene_tabs_rect
        pos = (tab_rect.x + 5, tab_rect.y + 5)
        r = pygame.Rect(tab_rect.x, tab_rect.y, 80, tab_rect.height)
    ...
    def test_dispatch_right_click_canvas(self, make_app):
        app = make_app()
        sr = app.layout.canvas_rect
    ...
    def test_dispatch_middle_click_tab(self, make_app):
        app = make_app(extra_scenes=True)
        tab_rect = app.layout.scene_tabs_rect
        r = pygame.Rect(tab_rect.x, tab_rect.y, 80, tab_rect.height)
        app.tab_hitboxes = [(r, 0, "main")]
    ...
    def test_dispatch_keydown_escape_context_menu(self, make_app):
        app = make_app()
...
class TestBuildPresetActions:
    def test_build_actions(self, make_app):
        app = make_app()
...
class TestMainModule:
    def test_main_function(self, tmp_path, monkeypatch, make_app):
...
class TestNonAppModules:
    ...
    def test_overlays_dead_code(self, make_app):
        """overlays.py L381-427 is dead code in _draw_perf_overlay fallback."""
        # Skip — dead code path

    def test_panels_miss_lines(self, make_app):
        """panels.py L231-232, L295-296."""
        app = make_app(widgets=[_w(type="gauge", value=50)])
        app.state.selected = [0]
        app.state.selected_idx = 0
        # Force draw to hit inspector gauge-specific lines
        app._draw_frame()

    def test_canvas_miss(self, make_app):
    ...
    def test_text_miss_lines(self, make_app):
        """text.py L60, 68-69, 110, 181-182."""
        app = make_app(widgets=[_w(text="A" * 200)])
        app.state.selected = [0]
        app._draw_frame()

    def test_focus_nav_miss(self, make_app):
    ...
    def test_input_handlers_miss(self, make_app):
