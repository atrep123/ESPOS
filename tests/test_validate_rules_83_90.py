...
def test_r84_icon_char_on_icon_ok():
    d = _make([{"type": "icon", "x": 0, "y": 0, "width": 16, "height": 16, "icon_char": "A"}])
    ws = [w for w in _warns(d) if "icon_char" in w.message and "non-icon" in w.message]
    assert ws == []
...
def test_r85_max_lines_on_gauge_warns():
    d = _make(
        [{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 20, "value": 50, "max_lines": 3}]
    )
...
def test_r85_max_lines_on_box_warns():
    d = _make([{"type": "box", "x": 0, "y": 0, "width": 40, "height": 20, "max_lines": 2}])
    ws = [w for w in _warns(d) if "max_lines" in w.message and "non-text" in w.message]
    assert len(ws) == 1
...
def test_r85_max_lines_on_label_no_warn():
    d = _make(
        [{"type": "label", "x": 0, "y": 0, "width": 50, "height": 12, "text": "hi", "max_lines": 2}]
    )
    ws = [w for w in _warns(d) if "max_lines" in w.message and "non-text" in w.message]
    assert ws == []
...
def test_r85_max_lines_zero_on_gauge_no_warn():
    ...
    d = _make(
        [{"type": "gauge", "x": 0, "y": 0, "width": 40, "height": 20, "value": 50, "max_lines": 0}]
    )
...
def test_r88_max_lines_with_wrap_ok():
    ...
    ws = [
        w
        for w in _warns(d)
        if "max_lines" in w.message and "text_overflow" in w.message and "ignored" in w.message
    ]
    assert ws == []
...
def test_r88_non_text_type_no_warn():
    ...
    ws = [
        w
        for w in _warns(d)
        if "text_overflow" in w.message and "max_lines" in w.message and "ignored" in w.message
    ]
    assert ws == []
...
def test_r89_entry_not_dict_errors():
    ...
    es = [
        e
        for e in _errors(d)
        if "responsive_rules[0]" in e.message and "must be a dict" in e.message
    ]
    assert len(es) == 1
...
def test_r89_entry_missing_condition_errors():
    ...
    es = [
        e
        for e in _errors(d)
        if "responsive_rules[0]" in e.message and "missing 'condition'" in e.message
    ]
    assert len(es) == 1
        ...
        [
            {
                ...
                "height": 12,
                "text": "hi",
                "responsive_rules": [{"condition": "w>100", "width": 80}],
            }
        ]
    )
