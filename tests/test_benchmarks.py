...
class TestJsonBenchmarks:
    def test_save_small(self, benchmark, tmp_path):
        ...
        for i in range(5):
            s.widgets.append(
                WidgetConfig(type="button", x=i * 20, y=10, width=18, height=10, text=f"B{i}")
            )
...
class TestCodegenBenchmarks:
    def test_codegen_small(self, benchmark, small_scene_json):
        benchmark(
            generate_ui_design_pair, small_scene_json, scene_name="bench", source_label="bench"
        )

    def test_codegen_medium(self, benchmark, medium_scene_json):
        benchmark(
            generate_ui_design_pair, medium_scene_json, scene_name="bench", source_label="bench"
        )

    def test_codegen_large(self, benchmark, large_scene_json):
        benchmark(
            generate_ui_design_pair, large_scene_json, scene_name="bench", source_label="bench"
        )
