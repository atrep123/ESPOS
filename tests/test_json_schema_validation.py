...
def test_valid_widget_types(schema):
    for wtype in [
        ...
        "panel", "icon", "chart", "list", "toggle",
    ]:
        d = _minimal_design([{
            "type": wtype, "x": 0, "y": 0, "width": 20, "height": 10,
        }])
...
def test_invalid_widget_type(schema):
    d = _minimal_design([{
        "type": "not_a_widget", "x": 0, "y": 0, "width": 20, "height": 10,
    }])
...
def test_width_must_be_integer(schema):
    d = _minimal_design([{
        "type": "label", "x": 0, "y": 0, "width": "twenty", "height": 10,
    }])
    assert not _is_valid(d, schema)
...
def test_x_must_be_integer(schema):
    d = _minimal_design([{
        "type": "label", "x": "zero", "y": 0, "width": 20, "height": 10,
    }])
    assert not _is_valid(d, schema)
...
def test_checked_must_be_boolean(schema):
    d = _minimal_design([{
        "type": "checkbox", "x": 0, "y": 0, "width": 14, "height": 14,
        "checked": "yes",
    }])
    assert not _is_valid(d, schema)
...
def test_border_must_be_boolean(schema):
    d = _minimal_design([{
        "type": "label", "x": 0, "y": 0, "width": 40, "height": 12,
        "border": 1,
    }])
    assert not _is_valid(d, schema)
...
def test_width_minimum_1(schema):
    d = _minimal_design([{
        "type": "label", "x": 0, "y": 0, "width": 0, "height": 10,
    }])
    assert not _is_valid(d, schema)
...
def test_height_minimum_1(schema):
    d = _minimal_design([{
        "type": "label", "x": 0, "y": 0, "width": 10, "height": 0,
    }])
    assert not _is_valid(d, schema)
...
def test_x_minimum_0(schema):
    d = _minimal_design([{
        "type": "label", "x": -1, "y": 0, "width": 10, "height": 10,
    }])
    assert not _is_valid(d, schema)
