...
def _errs(data):
    return [i for i in validate_data(data, file_label="test", warnings_as_errors=False) if i.level == "ERROR"]


def _warns(data):
    return [i for i in validate_data(data, file_label="test", warnings_as_errors=False) if i.level == "WARN"]
...
class TestRule115:
    ...
    def test_has_slider(self):
        d = _make([
            {"type": "slider", "x": 0, "y": 0, "width": 80, "height": 16,
    ...
    def test_focusable_but_invisible(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16, "text": "X",
             "visible": False},
        ])
        ws = [w for w in _warns(d) if "no focusable" in w.message]
        assert len(ws) == 1
...
class TestRule116:
    ...
    def test_gauge_not_affected(self):
        ...
        d = _make([
            {"type": "gauge", "x": 0, "y": 0, "width": 80, "height": 40,
             "min_value": 100, "max_value": 50},
        ])
        es = [e for e in _errs(d) if "chart" in e.message and "min_value" in e.message]
        assert es == []
...
class TestRule117:
    ...
    def test_progressbar_height_ok(self):
        d = _make([
            {"type": "progressbar", "x": 0, "y": 0, "width": 80, "height": 10,
             "value": 50, "min_value": 0, "max_value": 100},
        ])
    ...
    def test_progressbar_height_3_ok(self):
        d = _make([
            {"type": "progressbar", "x": 0, "y": 0, "width": 80, "height": 3,
             "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "progressbar" in w.message and "fill" in w.message]
        assert ws == []
    ...
    def test_progressbar_height_2(self):
        d = _make([
            {"type": "progressbar", "x": 0, "y": 0, "width": 80, "height": 2,
             "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "progressbar" in w.message and "fill" in w.message]
        assert len(ws) == 1
    ...
    def test_progressbar_height_1(self):
        d = _make([
            {"type": "progressbar", "x": 0, "y": 0, "width": 80, "height": 1,
             "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "progressbar" in w.message and "fill" in w.message]
        assert len(ws) == 1
...
class TestRule118:
    ...
    def test_valid_keys(self):
        d = _make([
            {"type": "label", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "Hi", "constraints": {"ax": "center", "sy": True}},
        ])
    ...
    def test_all_valid_keys(self):
        d = _make([
            {"type": "label", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "Hi", "constraints": {"b": "top", "ax": "c", "ay": "m",
                                            "sx": True, "sy": False, "mx": 5,
                                            "my": 5, "mr": 10, "mb": 10}},
        ])
        ws = [w for w in _warns(d) if "unrecognized keys" in w.message]
        assert ws == []
    ...
    def test_multiple_bad_keys(self):
        d = _make([
            {"type": "label", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "Hi", "constraints": {"foo": 1, "bar": 2, "ax": "c"}},
        ])
        ws = [w for w in _warns(d) if "unrecognized keys" in w.message]
        assert len(ws) == 1
...
class TestRule121:
    ...
    def test_gauge_with_text_ok(self):
        d = _make([
            {"type": "gauge", "x": 0, "y": 0, "width": 80, "height": 20,
             "text": "CPU", "value": 50, "min_value": 0, "max_value": 100},
        ])
    ...
    def test_gauge_with_text_height_8(self):
        d = _make([
            {"type": "gauge", "x": 0, "y": 0, "width": 80, "height": 8,
             "text": "CPU", "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "too short to render text" in w.message]
        assert ws == []
    ...
    def test_gauge_with_text_height_7(self):
        d = _make([
            {"type": "gauge", "x": 0, "y": 0, "width": 80, "height": 7,
             "text": "CPU", "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "too short to render text" in w.message]
        assert len(ws) == 1
    ...
    def test_progressbar_with_text_short(self):
        d = _make([
            {"type": "progressbar", "x": 0, "y": 0, "width": 80, "height": 5,
             "text": "Load", "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "too short to render text" in w.message]
        assert len(ws) == 1
    ...
    def test_chart_with_text_short(self):
        d = _make([
            {"type": "chart", "x": 0, "y": 0, "width": 80, "height": 6,
             "text": "Stats", "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "too short to render text" in w.message]
        assert len(ws) == 1
    ...
    def test_gauge_no_text_no_warning(self):
        d = _make([
            {"type": "gauge", "x": 0, "y": 0, "width": 80, "height": 5,
             "value": 50, "min_value": 0, "max_value": 100},
        ])
        ws = [w for w in _warns(d) if "too short to render text" in w.message]
        assert ws == []
...
class TestRule122:
    ...
    def test_valid_runtime_keys(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "bind=sensor.temp;kind=int;min=0;max=100;step=1"},
        ])
    ...
    def test_valid_key_alias(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "key=sensor.temp;type=bool;values=off|on"},
        ])
        ws = [w for w in _warns(d) if "recognized meta key" in w.message]
        assert ws == []
    ...
    def test_unknown_key(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "txt=sensor.temp;kind=int"},
        ])
        ws = [w for w in _warns(d) if "recognized meta key" in w.message]
        assert len(ws) == 1
    ...
    def test_multiple_unknown_keys(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "foo=bar;baz=qux"},
        ])
        ws = [w for w in _warns(d) if "recognized meta key" in w.message]
        assert len(ws) == 2
    ...
    def test_case_insensitive(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "Bind=sensor.temp;Kind=int"},
        ])
        ws = [w for w in _warns(d) if "recognized meta key" in w.message]
        assert ws == []
    ...
    def test_empty_parts_skipped(self):
        d = _make([
            {"type": "button", "x": 0, "y": 0, "width": 80, "height": 16,
             "text": "X", "runtime": "bind=x;;kind=int;"},
        ])
        ws = [w for w in _warns(d) if "recognized meta key" in w.message]
        assert ws == []
