...
class TestAuditDesignsMain:
    ...
    def test_valid_file_exits_0(self, tmp_path, monkeypatch, capsys):
        design = {"width": 256, "height": 128, "scenes": {"main": {"widgets": [{"type": "label"}]}}}
    ...
    def test_custom_max_dimensions(self, tmp_path, monkeypatch, capsys):
        design = {"width": 400, "height": 300, "scenes": {"main": {"widgets": []}}}
        (tmp_path / "big.json").write_text(json.dumps(design), encoding="utf-8")
        from tools.audit_designs import main

        with patch(
            "sys.argv",
            ["audit_designs", "--root", str(tmp_path), "--max-width", "500", "--max-height", "500"],
        ):
            with pytest.raises(SystemExit) as exc:
                main()
            assert exc.value.code == 0
    ...
    def test_valid_json_exports(self, tmp_path, capsys):
        design = {
            ...
            "scenes": {
                "main": {
                    "widgets": [
                        {"type": "label", "x": 0, "y": 0, "width": 40, "height": 14, "text": "hi"}
                    ]
                }
            },
        }
        ...
        with patch("sys.argv", ["ui_export_c_header", str(json_path), "-o", str(out_path)]):
            main()
...
class TestLivePreviewMain:
    ...
    def test_valid_json_calls_send(self, tmp_path, monkeypatch):
        ...
        with patch(
            "sys.argv", ["live_preview", str(json_path), "--port", "COM99", "--baud", "9600"]
        ):
            main()
        assert len(sent) == 1
